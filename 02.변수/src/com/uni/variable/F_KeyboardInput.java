...
public class F_KeyboardInput {
	...
	public void inputScanner() {
		Scanner sc = new Scanner(System.in);// -> 입력받은 값을 바이트 단위로 받아 들이겠다.

		System.out.println("당신의 이름은 무엇입니까?");

		// 사용자가 입력한 값을 문자열로 받아오는 매소드(next(), nextLine())

		// String name = sc.next();// 공백이 있는경우 공백 이전까지 읽어온다 즉, 띄어쓰기 전까지
		String name = sc.nextLine();// 사용자가 입력한 값 모두 읽어온다

		System.out.println("당신의 나이는 몇살입니까?");

		int age = sc.nextInt();// 사용자가 입력한 정수에 대한 값만 읽어온다.

		System.out.println("당신의 키는 몇입니까.");

		double height = sc.nextDouble();// 사용자가 입력한 실수에 대한 값만 읽어온다.

		System.out.println(name + "님은" + age + "세이며,키는" + height + "입니다");
	...
	public void inputScanner2() {
		...
		System.out.printf("%s 님은" + "%d 세이며,사는 곳 은"+ "%s 이고,키는"  + "%2f + 입니다",name,age,address,height);
	...
	public void inputScanner3() {
	   ...
	   //sc.nextchar() -> 존재하지 않는다. CharAt : 문자열에서 지정된 위치에 있는 문자하나만 뽑아내는 기능을 함. 이때 위치는 제일 앞문자가 0부터 시작해서 1씩 증가
